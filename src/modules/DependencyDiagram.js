...
treeviewSpanIndentStyle["marginLeft"] = "1px"; // originally 10px
treeviewSpanIndentStyle["marginRight"] = "1px"; // 10px

let treeviewSpanIconStyle = treeviewSpanStyle;
treeviewSpanIconStyle["marginLeft"] = "1px"; // 10px
treeviewSpanIconStyle["marginRight"] = "1px"; // 5px
  ...
  setNodeId(node) {
    ...
    return node.nodes.map(childNode => {
      return {
        ...
        gloss: childNode.gloss,
        grammar: childNode.grammar,
        lemma: childNode.lemma,
        refersTo: childNode.refersTo,
        icon: childNode.icon
      }
    });
  ...
  render() {
    ...
    if (options.enableLinks) {
      ...
      if (isNaN(node.key)) {
        ...
        if (this.props.options.offsetKey) {
          key++;
        }
      }
      let refersTo = node.refersTo;
      if (refersTo) {
        if (!isNaN(refersTo)) {
    ...
    let badges;
    if (options.showTags && node.grammar) {
      badges = node.grammar.map(function (tag) {
        return (
